...
- Switched storing the state of the main window to the window state persistence API
  provided natively by Electron. This means that Skybrush Viewer will start with its
...
- Fixed a crash that happened when trajectory validation was invoked from Blender
  when the proximity chart was already visible in the chart window.
...
- Auto-updater logic is now wired properly to main window.

## [2.10.3] - 2026-08-14

### Fixed

- Added ZIP format for macOS build because Squirrel.Mac needs it.

## [2.10.2] - 2026-08-14

### Fixed

- Added deferred import for the auto-updater module to prevent problems when the app
  is packaged with Electron. No user-facing changes.

## [2.10.1] - 2026-07-28

### Changed

- Preparing for supporting automatic updates. No user-facing changes.
...
- Fixed incorrect handling of Boolean props in the `<a-drone-flock>` component,
  which made the "Show label" and "Show yaw" switches unresponsive.
